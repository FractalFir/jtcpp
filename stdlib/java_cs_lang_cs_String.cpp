...
#include <cstring>
java_cs_lang_cs_String::java_cs_lang_cs_String(const char16_t* buffer,size_t length){
    char16_t* new_buffer = new char16_t[length];
    std::memcpy(new_buffer,buffer,length*sizeof(char16_t));
    this->buffer = new_buffer;
    this->length = length;
}
java_cs_lang_cs_String::java_cs_lang_cs_String(const char16_t* null_terminated_buffer){
