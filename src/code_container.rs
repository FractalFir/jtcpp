...
use crate::{
    Class, ClassRef, FatClass, FieldRef, IString, Method, MethodRef, Object, ObjectRef, StaticRef,
};
...
impl CodeContainer {
    pub(crate) fn class_alias(&mut self, class_name: &str, prefix: &str) {
        ...
        match self.class_names.get(class_name) {
            Some(id) => {
                self.class_names
                    .insert(format!("{prefix}{class_name}").into(), *id);
            }
            None => (),
        }
    }
    ...
    pub(crate) fn diagnose_method(&self, method_id: usize) {
        ...
        let dep = if let Some(dep) = self.method_dependency(method_id) {
            dep
        } else {
        ...
        println!(
            "method with name {name} and ID {method_id} is missing, because it depends on {dep}!"
        );
        let mut dep = dep;
        //let mut limiter = 0;
